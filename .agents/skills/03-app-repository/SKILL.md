...
description: 03 APP 客户端仓库入口；在 03desktop 的包名、namespace、版本、签名、远端或发布产物任务中调用共享的 03 APP 管家规则。
---

# 03 APP 仓库入口

本仓库登记为 `productId=03desktop`。这是路由 skill，不复制身份规则正文。
...
3. 本产品固定包名和 namespace 为 `com.ninepointnine.desktop`，`licenseMode=none`；不得复用 03lyrics、03cast 或 03helper 的身份、签名和许可证。
