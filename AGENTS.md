...
9. V1 架构方案与决策 0002 已确认；默认在同一个施工对话内连续完成整个 V1。施工过程只保留 3 个内部技术检查点来控制依赖顺序和验证风险，检查点不是独立交付阶段，不中断施工、不提前交给用户验收、不创建阶段提交。
...
9. V1 连续施工依次经过 3 个内部技术检查点：①悬浮抽屉与恢复基础；②应用目录与管理主链；③APK 安装、快捷控制与整体收口。每个检查点完成后执行相关自动检查、测试、Debug 构建，并在车机在线且当前能力可运行时自动覆盖安装和执行最短 smoke，然后继续下一个检查点。
10. 内部检查点只压缩无关回归范围，不得省略当前环境可自动发现的编译错误、相关单测、安装失败、启动崩溃或该检查点主路径错误；检查失败必须原地修复后再继续。
11. 3 个检查点全部完成后，再执行完整构建、最终覆盖安装和 V1 最短全链路 smoke，一次性交给用户做更广的视觉、连续操作和真实车辆场景体验；用户反馈问题时继续修复，用户确认无问题或明确要求提交后才能提交。
...
3. V1 连续施工期间不因内部检查点暂存、提交或请求用户验收；整个 V1 完成并安装最终 Debug 版本后才交付用户扩展验收。用户明确回复无问题、要求提交或在任务开始时已授权本轮提交时，才执行 `git add` 和提交。
