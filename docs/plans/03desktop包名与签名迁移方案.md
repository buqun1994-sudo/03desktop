...
displayName = 03桌面
testPackageName = com.ninepointnine.desktop.test
releaseVersionSource = release-version.properties
```
...
1. Android `namespace`、Kotlin 主源码和协议包保持 `com.ninepointnine.desktop`；正式 `applicationId` 使用正式包名，Debug/staging 通过 `applicationIdSuffix=".test"` 派生 `com.ninepointnine.desktop.test`。Debug instrumentation、JVM 测试、安装脚本和配置护栏按目标变体读取实际包名。
...
默认 Debug 使用开发机自动 Debug 证书；测试包的包名和版本后缀始终生效，不依赖签名材料。需要长期测试签名时，再显式注入 staging 材料。
...
所有变体都从仓库根目录 `release-version.properties` 读取 `releaseVersionName` 和 `releaseVersionCode`。Debug/staging 只追加 `-test` 版本名后缀，测试包与正式包共用版本码；递增该文件即可连续覆盖更新测试包，不创建第二套版本文件。
...
4. 测试包 `com.ninepointnine.desktop.test` 与正式包 `com.ninepointnine.desktop` 可在同一设备并存；测试包只能覆盖同一测试包身份，正式包只能覆盖同一正式包身份。长期测试设备固定使用测试包 / staging 证书，正式用户固定使用 production 包 / 证书。

## 7. 验证标准

1. 默认 Debug 构建不依赖仓库外签名材料。
2. staging / production 缺少各自属性文件时必须失败。
3. staging Debug 的 APK 包名必须为 `com.ninepointnine.desktop.test`、版本名为正式版本追加 `-test`；production Release 的 APK 包名必须为 `com.ninepointnine.desktop`、版本名为正式版本原值；两者 `versionCode` 均来自同一 `release-version.properties`。
