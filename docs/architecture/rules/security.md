...
3. 触发条件：测试或正式发布。动作：正式包固定使用 `com.ninepointnine.desktop` 与 production 证书；测试包固定使用 `com.ninepointnine.desktop.test` 与 `-test` 版本名后缀，版本值来自同一 `release-version.properties`，证书仓库外备份。验证：按目标变体核对包名、版本、签名摘要与升级安装 smoke。边界：未经明确指令不得生成、替换、提交或分发签名材料。
4. 触发条件：日志或测试证据。动作：只记录状态类别、计数和必要组件，不记录个人文件路径、APK 内容或密钥。验证：提交前敏感词和本机路径扫描。边界：ADB 原始日志、截图和录屏默认不提交。
5. 触发条件：修改全局返回。动作：保持既有 AccessibilityService 组件名，服务只连接 `GlobalBackActionGateway` 并调用 `GLOBAL_ACTION_BACK`；触发条长按回桌面必须在 OverlayService 使用公开 HOME Intent，不得扩展服务动作。验证：网关单测、手势状态单测、配置检查、Debug 构建和真机轻点/长按/滑动分流。边界：不读取无障碍事件、窗口树或输入，不执行手势，不增加 RECENTS、媒体和自动点击。
