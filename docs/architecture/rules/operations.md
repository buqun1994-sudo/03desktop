...
3. 触发条件：完成可在目标车机手测的应用行为改动。动作：构建通过后执行 `./scripts/install-debug-to-device.sh`，脚本只允许向型号、SDK 和分辨率均匹配的车机覆盖安装仓库固定 Debug APK，再非阻塞启动 `com.tcrrry.desktop/.MainActivity`，并在有限时间内轮询进程与悬浮服务。验证：安装成功、目标包更新时间更新、进程和前台悬浮服务存在，脚本能明确成功或超时退出。边界：入口 Activity 可能立即结束，不得用无界界面启动等待代替运行状态检查；不清数据、不授予权限、不重启设备，不接受自定义包名或任意 APK 路径。
