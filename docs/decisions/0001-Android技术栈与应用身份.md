...
4. 正式 `applicationId` 与 `namespace` 固定为 `com.ninepointnine.desktop`；Debug/staging 通过 `applicationIdSuffix=".test"` 使用独立测试包 `com.ninepointnine.desktop.test`，namespace 和源码包不变。
5. staging 与 production 分别使用 `03桌面` 独立证书；证书不进入仓库。所有变体共用 `release-version.properties`，测试版本名追加 `-test`，测试包与正式包各自在同一包身份内保持 `versionCode` 单调递增。
...
3. 历史包 `com.tcrrry.desktop` 不能被新包覆盖；正式包与测试包也不能互相覆盖，均需在各自包身份内升级。进入 production 身份后，正式网页更新只使用同一 production 证书并递增共享版本文件中的 `versionCode`。
